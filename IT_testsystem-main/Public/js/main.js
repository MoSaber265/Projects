...
async function initAdvisorDashboard() {
    ...
    if (document.getElementById('advisorName')) {
        document.getElementById('advisorName').innerText = `Advisor: ${user.name}`;
    }
}
...
async function searchStudentForAdvisor() {
    ...
    const searchType = window.advisorSearchMode || 'universityId';

    if (!searchValue) return alert('Please enter a search value first');

    const endpoint = `/api/advisor/search-student?${encodeURIComponent(searchType)}=${encodeURIComponent(searchValue)}`;
    const data = await apiCall(endpoint);

    if (!data || !data.success) {
        showMessage(data?.message || 'Student not found. Please verify your input.', true);
        return;
    }
    ...
    const enrollmentCourses = data.enrollments || [];

    window.currentAdvisorStudentId = student._id;
    document.getElementById('studentDetailView').classList.remove('hidden');
    document.getElementById('advisorStudentIdText').innerText = `Student ID: ${student.universityId || 'N/A'}`;
    document.getElementById('advisorStudentName').value = student.name || '';
    document.getElementById('advisorStudentEmail').value = student.email || '';
    document.getElementById('advisorStudentGPA').value = data.studentInfo?.gpa || '';
    document.getElementById('advisorStudentDepartment').value = data.studentInfo?.department || '';
    if (document.getElementById('advisorDeleteButton')) {
        document.getElementById('advisorDeleteButton').disabled = false;
    }

    const coursesBody = document.getElementById('advisorCoursesBody');
    if (coursesBody) {
        if (enrollmentCourses.length === 0) {
            coursesBody.innerHTML = '<tr><td colspan="4">No enrolled courses found.</td></tr>';
        } else {
            coursesBody.innerHTML = enrollmentCourses.map(course => `
                <tr>
                    <td>${course.courseId?.title || 'Unknown'}</td>
                    <td>${course.courseId?.code || 'N/A'}</td>
                    <td>${course.courseId?.credits ?? 'N/A'}</td>
                    <td><button class="btn-action-danger" onclick="dropCourse('${student._id}', '${course.courseId?._id || ''}')">Drop</button></td>
                </tr>
            `).join('');
        }
    }
